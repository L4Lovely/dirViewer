...
TABS[tab]._drawTab()
TABS[tab]._drawList()
TABS[tab]._drawSizeList()
screen.refresh()

def DEBUG():
    screen.addstr(14,14,str(len(TABS[tab].currDir)))
...
def on_press(key):
    ...
    if key == Key.up and TABS[tab].cPos_relative != 0:
        ...
        if (TABS[tab].cPos[1] +1 < termHeight -1 and TABS[tab].cPos[1]< len(TABS[tab].currDir)+2):    
            TABS[tab].cPosPre[0] = TABS[tab].cPos[0]; TABS[tab].cPosPre[1] = TABS[tab].cPos[1]
            TABS[tab].cPos[1] +=1
            TABS[tab].cPos_relative += 1
            TABS[tab]._setCursor()
            screen.refresh()
        elif(TABS[tab].cPos_relative +1< len(TABS[tab].currDir)):
            TABS[tab].cPos_relative +=1
            TABS[tab]._clearList()
            TABS[tab]._drawList()
            screen.refresh()
